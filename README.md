...
`group_unhandled_paths`: extends `filter_unhandled_paths`, but instead of ignoring the requests, it groups them under the `__unknown__` path. This option overrides `filter_unhandled_paths` and set it to `True`. Default is `False`.
